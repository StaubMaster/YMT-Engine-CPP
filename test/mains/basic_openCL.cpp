...
int main(void)
{
	...
	std::string kernel2{R"CLC(
		...
		kernel void vectorAdd(
			global const Foo* aNum,
			global const int *inputA,
			global const int *inputB,
			global int *output,
			int val,
			write_only pipe int outPipe,
			queue_t childQueue
			)
		{
			...
			^{
				...
				inputB[get_global_size(0)*2 + get_global_id(0)] +
				globalA;
			});
			...
			^{
				...
				inputA[get_global_size(0)*3 + get_global_id(0)] +
				inputB[get_global_size(0)*3 + get_global_id(0)] +
				globalA + 2;
			});
		}
	)CLC"};
